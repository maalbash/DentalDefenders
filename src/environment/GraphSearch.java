...
public class GraphSearch
{
    ...
    {
        return graphSearch(startNode, endNode, graph, GameConstants.searchType.ASTAR);
    }

    public ArrayList<Integer> dijkstraSearch(int startNode, int endNode, Graph graph)
    {
        return graphSearch(startNode, endNode, graph, GameConstants.searchType.DIJKSTRA);
    }


    public ArrayList<Integer> graphSearch(int startNode, int endNode, Graph graph, GameConstants.searchType mode)
    {
        ...
        fMap.put(startNode, 1.01f * heuristicCost(startNode, endNode, graph.nodeMap, graph.edgeWeight, GameConstants.DEFAULT_HEURISTIC));
        ...
        while (!openList.isEmpty())
        {
            ...
            for (Edge edge : edges)
            {
                ...
                if (!gMap.containsKey(currentNeighbor) || newCost < gMap.get(currentNeighbor))
                {
                    ...
                    fMap.put(currentNeighbor,
                            newCost + heuristicWeight * heuristicCost(currentNeighbor, endNode, graph.nodeMap, graph.edgeWeight, GameConstants.DEFAULT_HEURISTIC));
    ...
    float heuristicCost (int node1, int node2, Map<Integer, Node> map, float edgeWeight, GameConstants.heuristic heuristic)
    {
        switch (heuristic)
        {
            case MANHATTAN:
